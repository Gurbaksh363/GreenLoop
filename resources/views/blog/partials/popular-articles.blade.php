    ...
    <div class="space-y-4">
        @foreach ($popularPosts as $article)
            <a href="{{ route('public.blog.show', $article->slug) }}" class="flex gap-4 group items-start">
                <img src="{{ $article->featured_image ? Storage::url($article->featured_image) : 'https://images.unsplash.com/photo-1574943320219-553eb213f72d?ixlib=rb-4.0.3&auto=format&fit=crop&w=200&q=80' }}" class="w-16 h-16 rounded-lg object-cover flex-shrink-0 bg-gray-100">
